...
units = {'POWER': 'Power (W)', 'FREQ': 'Frequency (Hz)'}
...
def plot_event_timeline(event_timeline, power_measurements, columns, title):
  ...
  power_data['TIME_STAMP'] = power_data['TIME_STAMP'] - power_data['TIME_STAMP'].min()
  for event in event_timeline:
    event['start'] = event['start'] / 1e9 #ns to s
    event['end'] = event['end'] / 1e9
    event['duration'] = event['duration'] / 1e9
  ...
  if columns[0] in power_data.columns:
    column = columns[0]
    ax2 = ax.twinx()
    ax2.set_ylim(0, power_data[column].max() + (power_data[column].max()) * .05) # arbritrary 5% buffer on top of max point. 
    ax2.yaxis.tick_left()
    ax2.yaxis.set_label_position("left")
    ax2.plot(power_data['TIME_STAMP'], power_data[column], picker=False, color="red")
  ...
  if len(columns) > 1:
    if columns[1] in power_data.columns:
      column = columns[1]
      ax3 = ax.twinx()
      ax3.set_ylim(0, power_data[column].max() + (power_data[column].max()) * .05) # arbritrary 5% buffer on top of max point. 
      ax3.yaxis.tick_right()
      ax3.yaxis.set_label_position("right")
      ax3.plot(power_data['TIME_STAMP'], power_data[column], picker=False, color="blue")
  ...
  for i, event in enumerate(event_timeline):
    ...
    text = ax4.text(x_start + bar_width / 2, y_start + bar_height / 2, event['name'], ha='center', va='center', color='white')
    bounding_box = text.get_window_extent(renderer=renderer)
    text_width = ax4.transData.inverted().transform((bounding_box.width, 0))[0] - ax4.transData.inverted().transform((0, 0))[0]
    ...
    ax4.broken_barh([(x_start, bar_width)], (y_start, bar_height), facecolors=bar_colour, picker=True, label=legend_label)

  ax4.set_ylim(0, max([event['depth'] for event in event_timeline]) * 1.5) # have bars only take up bottom half of plot
  ax4.set_yticklabels([])
  ax4.set_yticks([]) 
  ...
  def hover(event):
    ...
    prev_annotation = ax4.annotate(label, xy=(event.mouseevent.xdata,event.mouseevent.ydata),xytext=((ax4.get_xlim()[1] / 3) , ax4.get_ylim()[1] - 0.5), arrowprops=dict(arrowstyle="->",color="black",lw=1))
    fig.canvas.draw()
...
timeline = generate_stack_timeline('./fake_trace.json')
...
plot_event_timeline(timeline, 'main_test1731268056.txt',["POWER"], "CPUx Power Against Trace Timeline")